 ...
 *
 * @brief USB HID TOUCHPAD profile implementation.
 *
 * @file usb_hid_touchpad.c
 *
...
/**
 * \brief   Build touchpad HID report payload.
 *
 * \details Fills a 6-byte touchpad report with tip and X/Y coordinate fields.
 *
...
{
	if (report == NULL || report_len < USB_HID_TOUCHPAD_REPORT_SIZE) {
		return -EINVAL;
	}

	memset(report, 0, USB_HID_TOUCHPAD_REPORT_SIZE);
	report[0] = USB_HID_TOUCHPAD_REPORT_ID;
