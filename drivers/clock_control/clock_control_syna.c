...
syna_clk_read(const struct clock_control_syna_config *config, mm_reg_t addr)
{
	return sys_read32(config->regs + addr);
}

static inline void
syna_clk_write(const struct clock_control_syna_config *config, uint32_t data,
	       mm_reg_t addr)
{
...
static int
syna_get_axi_clock(uint32_t id)
{
	int clock_id = -1;
...
static uint32_t
syna_get_cgl_offset(const struct clock_control_syna_config *config, uint32_t id)
{
	uint32_t offset;
...
static int
syna_clk_set_rate(const struct device *dev, uint32_t id, uint32_t rate)
{
	...
	value = sys_read32(config->regs + offset);
	value &= ~(0x1f << 2);
...
static uint32_t
syna_clk_get_rate(const struct device *dev, uint32_t id)
{
	const struct clock_control_syna_config *config = dev->config;
	uint32_t offset = syna_get_cgl_offset(config, id);
	uint32_t parent_rate = PLL0_RATE;
	uint32_t divider = 1, sel_val = 0;
	uint32_t value;
	...
	value = sys_read32(config->regs + offset);
	if (value & (1 << 3)) { /* div3 switch */
		divider = 3;
	} else {
		if (value & (1 << 2)) { /* div switch */
...
static inline int
syna_clk_enable(const struct device *dev, uint32_t id, int enable)
{
	const struct clock_control_syna_config *config = dev->config;
	...
	if (axi_clock >= 0) {
		syna_clk_enable(dev, axi_clock, enable);
	}

	if (cgl) {
		value = sys_read32(config->regs + cgl);
		if (enable) {
			value |= 1;  /* enable */
		} else {
			value &= ~1; /* disable */
		}
		sys_write32(value, config->regs + cgl);
	}
...
static inline int
api_on(const struct device *dev, clock_control_subsys_t clkcfg)
{
...
static inline int
api_off(const struct device *dev, clock_control_subsys_t clkcfg)
{
	uint32_t clock_id = (uint32_t)(uintptr_t)clkcfg;
...
static int
api_get_rate(const struct device *dev, clock_control_subsys_t clkcfg,
	     uint32_t *rate)
{
	uint32_t clock_id = (uint32_t)(uintptr_t)clkcfg;
	uint32_t clk_rate;
...
static int
api_set_rate(const struct device *dev, clock_control_subsys_t clkcfg,
...
DEVICE_DT_INST_DEFINE(0, syna_clkctrl_init, NULL, NULL, &syna_config,
