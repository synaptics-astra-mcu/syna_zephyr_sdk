...
struct video_syna_sr100_enc_config {
	...
	size_t lp_src_offset;
	size_t lp_dst_offset;
	uint32_t mode;
							 ...
							 struct video_syna_sr100_enc_layout *layout)
{
	...
	input_offset = (size_t)cfg->lp_src_offset;
	output_offset = (size_t)cfg->lp_dst_offset;

	if ((input_offset >= cfg->lp_mem_size) || (output_offset >= cfg->lp_mem_size)) {
		return -ENOMEM;
	}

	if ((input_offset > (SIZE_MAX - raw_size)) || ((input_end = input_offset + raw_size) > cfg->lp_mem_size)) {
		return -ENOMEM;
	}

	if (output_offset < input_end) {
		return -ENOMEM;
	}
	...
	layout->input_offset = input_offset;
	layout->output_offset = output_offset;
	layout->jpeg_size_limit = jpeg_limit;
...
static int video_syna_sr100_enc_start(const struct device *dev)
{
	...
	if (ret != 0) {
		LOG_ERR("Invalid LP memory layout: %d", ret);
...
#define VIDEO_SYNA_SR100_ENC_INIT(n) \
	...
	static const struct video_syna_sr100_enc_config video_syna_sr100_enc_config_##n = { \
		...
		.lp_src_offset = DT_INST_PROP_OR(n, lp_src_offset, 0), \
		.lp_dst_offset = DT_INST_PROP_OR(n, lp_dst_offset, 0), \
