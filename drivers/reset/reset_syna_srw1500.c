...
#define DT_DRV_COMPAT syna_srw1500_reset
...
#ifndef SYNA_SECURE_RST
#define SYNA_SECURE_RST 0U
#endif

static inline uint32_t syna_reset_base(const struct reset_syna_config *config, uint32_t id)
{
	if ((id & SYNA_SECURE_RST) != 0U) {
		return config->base_sec;
	}
...
static int syna_reset_status(const struct device *dev, uint32_t id, uint8_t *status)
{
	...
	uint32_t base = syna_reset_base(config, id);
	uint32_t val;
	uint32_t rst_reg;
...
#define SYNA_RESET_INIT(n)                                                      \
	static const struct reset_syna_config reset_syna_cfg_##n = {            \
		.base = DT_INST_REG_ADDR_BY_NAME(n, global),                   \
		.base_sec = DT_INST_REG_ADDR_BY_NAME(n, global_sec),           \
	};                                                                      \
