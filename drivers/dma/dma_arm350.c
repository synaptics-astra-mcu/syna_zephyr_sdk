...
#define DMA350_XSIZE_FIELD_MAX	0xffffU
...
#define DMA_HAS_AUTO_RELOAD	BIT(1)
#define DMA_HAS_TRIGGER_INPUT	BIT(2)
#define DMA_HAS_ERROR_CALLBACK	BIT(3)

struct arm350_channel_ctx {
	uint8_t flags;
	uint8_t port;
	dma_callback_t dma_callback;
	void *cb_data;
	struct dma_block_config *desc_chain_head;
	struct dma_block_config *active_desc;
	uint16_t source_data_size;
	uint16_t dest_data_size;
	uint8_t complete_callback_en;
};
...
struct arm350_dma_data {
	...
	uint32_t channels;
	struct arm350_channel_ctx attr[DMA_MAX_PORTS];
};
...
typedef struct dma_trigger_in_transfer
{
	...
	uint8_t mode;
	uint8_t block_size;	/* Trigger block size units */
...
#define CH_STATUS_ALL_MASK	(CH_STATUS_DONE | CH_STATUS_ERR | CH_STATUS_DISABLED | \
				 CH_STATUS_STOPPED)
...
static int arm350_dma_config_trigger_input(const struct device *dev, int ch,
						dma_trigger_in_transfer_t *trig_in)
{
	...
	reg |= trig_in->block_size << TRIGINBLKSIZE_POS;
...
static void arm350_dma_channel_reset(const struct device *dev, int ch)
{
	const struct arm350_dma_cfg *cfg = dev->config;
	uint32_t reg;
...
static int arm350_dma_channel_to_port(const struct arm350_dma_cfg *config, struct arm350_dma_data *data,
			       uint32_t ch)
{
	if (data->is_dma250) {
...
static int arm350_dma_port_to_channel(const struct arm350_dma_cfg *config,
...
static int arm350_dma_alloc_channel(const struct arm350_dma_cfg *config,
...
static int arm350_dma_load_descriptor(const struct device *dev, int ch,
				    const struct dma_block_config *block,
				    uint16_t source_data_size, uint16_t dest_data_size)
{
	const struct arm350_dma_cfg *cfg;
	struct arm350_dma_data *data;
	mem_addr_t ch_base;
	uint32_t src_units;
	uint32_t dst_units;
	uint32_t reg;
	...
	if ((ch < 0) || ((uint32_t)ch >= data->channels)) {
		return -EINVAL;
	}

	if ((block == NULL) || (source_data_size == 0U) || (dest_data_size == 0U)) {
		return -EINVAL;
	}

	if ((block->block_size == 0U) ||
		((block->source_address % source_data_size) != 0U) ||
		((block->dest_address % dest_data_size) != 0U) ||
		((block->block_size % source_data_size) != 0U) ||
		((block->block_size % dest_data_size) != 0U)) {
		return -EINVAL;
	}

	src_units = block->block_size / source_data_size;
	dst_units = block->block_size / dest_data_size;

	if ((src_units == 0U) || (dst_units == 0U) ||
		(src_units > DMA350_XSIZE_FIELD_MAX) ||
		(dst_units > DMA350_XSIZE_FIELD_MAX)) {
		return -EINVAL;
	}

	ch_base = DMA_CH_OFFS(cfg, ch);

	sys_write32(CH_STATUS_CLEAR_ALL, ch_base + DMA350_CH_STATUS);
	sys_write32(block->source_address, ch_base + DMA350_CH_SRCADDR);
	sys_write32(block->dest_address, ch_base + DMA350_CH_DESADDR);
#ifdef CONFIG_DMA_64BIT
	sys_write32(block->source_address >> 32, ch_base + DMA350_CH_SRCADDRHI);
	sys_write32(block->dest_address >> 32, ch_base + DMA350_CH_DESADDRHI);
#endif

	reg = (src_units << SRC_POS) | (dst_units << DES_POS);
	sys_write32(reg, ch_base + DMA350_CH_XSIZE);
	reg = ((src_units >> 16) << SRC_POS) | ((dst_units >> 16) << DES_POS);
	sys_write32(reg, ch_base + DMA350_CH_XSIZEHI);

	return 0;
}

static void arm350_dma_release_channel(const struct arm350_dma_cfg *config,
				    struct arm350_dma_data *data, uint32_t ch)
{
	int i;
...
static void arm350_dma_channel_irq_handler(const struct device *dev, int ch, uint32_t status)
{
	const struct arm350_dma_cfg *cfg = dev->config;
	...
	if (status & CH_STATUS_DONE) {
		...
		if ((data->attr[ch].active_desc != NULL) &&
		    (data->attr[ch].active_desc->next_block != NULL)) {
			const struct dma_block_config *next = data->attr[ch].active_desc->next_block;
			int port = arm350_dma_channel_to_port(cfg, data, ch);
			int ret = arm350_dma_load_descriptor(dev, ch, next,
							   data->attr[ch].source_data_size,
							   data->attr[ch].dest_data_size);

			if (ret == 0) {
				data->attr[ch].active_desc = data->attr[ch].active_desc->next_block;
				ret = arm350_dma_start(dev, port);
			}
			if (ret == 0) {
				if (data->attr[ch].complete_callback_en) {
	...
	if (status & CH_STATUS_ERR) {
		int port = arm350_dma_channel_to_port(cfg, data, ch);
		uint32_t ctrl = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_CTRL);
		uint32_t srctrans = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_SRCTRANSCFG);
		uint32_t desttrans = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_DESTRANSCFG);
		uint32_t srcaddr = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_SRCADDR);
		uint32_t dstaddr = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_DESADDR);
		uint32_t xsize = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_XSIZE);
		uint32_t src_trig = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_SRCTRIGINCFG);
		uint32_t dst_trig = sys_read32(DMA_CH_OFFS(cfg, ch) + DMA350_CH_DESTRIGINCFG);

		LOG_ERR("DMA350 ERR: port=%d hw_ch=%d status=0x%08x ctrl=0x%08x xsize=0x%08x",
			port, ch, status, ctrl, xsize);
		LOG_ERR("DMA350 ERR regs: src=0x%08x dst=0x%08x srctrans=0x%08x desttrans=0x%08x",
			srcaddr, dstaddr, srctrans, desttrans);
		LOG_ERR("DMA350 ERR trig: srctrig=0x%08x desttrig=0x%08x scfg_trig_nsec=0x%08x",
			src_trig, dst_trig, sys_read32(cfg->regs + DMA350_SCFG_TRIGINSEC0));
	...
	if (notify_client && data->attr[ch].dma_callback) {
		...
		if ((port >= 0) && (port != DMA_INVALID_PORT)) {
			data->attr[ch].dma_callback(dev, data->attr[ch].cb_data, port, cb_status);
		}
	}
}
...
static int arm350_dma_config(const struct device *dev, uint32_t channel, struct dma_config *config)
{
	...
	if ((config->head_block->block_size == 0U) ||
		((config->head_block->source_address % config->source_data_size) != 0U) ||
		((config->head_block->dest_address %  config->dest_data_size) != 0U) ||
		((config->head_block->block_size % config->source_data_size) != 0U) ||
		((config->head_block->block_size %  config->dest_data_size) != 0U)) {
		return -EINVAL;
	}

	src_units = config->head_block->block_size / config->source_data_size;
	dst_units = config->head_block->block_size /  config->dest_data_size;
	...
	ch = arm350_dma_alloc_channel(cfg, data, channel,
	...
	arm350_dma_channel_reset(dev, ch);

	data->attr[ch].flags &= ~DMA_HAS_ERROR_CALLBACK;
	if (!config->error_callback_dis) {
		data->attr[ch].flags |= DMA_HAS_ERROR_CALLBACK;
	}
	data->attr[ch].dma_callback = config->dma_callback;
	data->attr[ch].cb_data = config->user_data;
	data->attr[ch].desc_chain_head = config->head_block;
	data->attr[ch].active_desc = config->head_block;
	data->attr[ch].source_data_size = config->source_data_size;
	data->attr[ch].dest_data_size = config->dest_data_size;
	data->attr[ch].complete_callback_en = config->complete_callback_en ? 1U : 0U;
	...
	if (data->attr[ch].flags & DMA_HAS_AUTO_RELOAD) {
	...
	src_x_size = src_units;
	dst_x_size = dst_units;
	...
	if ((data->attr[ch].flags & DMA_HAS_TRIGGER_INPUT) &&
	    config->channel_direction != MEMORY_TO_MEMORY) {
		trig_in.sel = channel;
		trig_in.type = 2; /* hardware */
		trig_in.mode = 3; /* peripheral-driven pacing for I2S */
		if (trig_in.is_source)
			trig_in.block_size = config->source_data_size - 1;
		else
			trig_in.block_size = config->dest_data_size - 1;
		LOG_DBG("DMA350 cfg: port=%u hw_ch=%d dir=%u trig_sel=%u trig_on=%s",
			channel, ch, config->channel_direction, trig_in.sel,
			trig_in.is_source ? "src" : "dst");

		return arm350_dma_config_trigger_input(dev, ch, &trig_in);
	}
...
#endif
{
	...
	ch = arm350_dma_port_to_channel(cfg, data, channel);
	if (ch < 0) {
		return ch;
	}

	if ((block.block_size == 0U) ||
		((block.source_address % data->attr[ch].source_data_size) != 0U) ||
		((block.dest_address % data->attr[ch].dest_data_size) != 0U) ||
		((block.block_size % data->attr[ch].source_data_size) != 0U) ||
		((block.block_size % data->attr[ch].dest_data_size) != 0U)) {
		return -EINVAL;
	}

	src_units = block.block_size / data->attr[ch].source_data_size;
	dst_units = block.block_size / data->attr[ch].dest_data_size;
	if ((src_units == 0U) || (dst_units == 0U) ||
		(src_units > DMA350_XSIZE_FIELD_MAX) ||
		(dst_units > DMA350_XSIZE_FIELD_MAX)) {
		return -EINVAL;
...
static int arm350_dma_start(const struct device *dev, uint32_t channel)
{
	...
	if (data->attr[ch].flags & DMA_HAS_ERROR_CALLBACK) {
	...
	LOG_DBG("DMA350 start: port=%u hw_ch=%d cmd=0x%x", channel, ch, reg);
...
static int arm350_dma_stop(const struct device *dev, uint32_t channel)
{
	...
	data->attr[ch].desc_chain_head = NULL;
	data->attr[ch].active_desc = NULL;
	data->attr[ch].source_data_size = 0U;
	data->attr[ch].dest_data_size = 0U;
	data->attr[ch].complete_callback_en = 0U;
	arm350_dma_release_channel(cfg, data, channel);
				 ...
				 struct dma_status *stat)
{
	...
	if ((dev == NULL) || (dev->config == NULL) || (dev->data == NULL) || (stat == NULL)) {
		return -EINVAL;
	}
...
static void arm350_dma_isr(const struct device *dev)
{
	...
	for (ch = 0; ch < data->channels; ch++) {
		...
		if (status & CH_STATUS_ALL_MASK) {
			arm350_dma_channel_irq_handler(dev, ch, status);
		}
	}
}
...
static int arm350_dma_init(const struct device *dev)
{
	...
	for (ch = 0; ch < data->channels; ch++) {
		...
		if (buildcfg1 & BIT(5)) {
			data->attr[ch].flags |= DMA_HAS_TRIGGER_INPUT;
		}
		if (buildcfg1 & BIT(9)) {
			data->attr[ch].flags |= DMA_HAS_AUTO_RELOAD;
		}
	...
	/* Program trigger security map from DT for current execution context. */
