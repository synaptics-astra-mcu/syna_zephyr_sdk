...
WEST_PATCH_BASE = Path(".")
WEST_PATCH_YAML = Path("scripts") / "patches.yml"
...
class Patch(WestCommand):
    ...
    def apply(self, args, yml, dst_mods=None):
        ...
        for patch_info in patches:
            mod = self.get_module_path(patch_info.get("module", "zephyr"))
            if mod is None:
                continue
            ...
            apply_cmd = patch_info.get("apply-command", "git apply")
            ...
            # Skip if already applied (check with --reverse dry-run)
            check_cmd_list = apply_cmd_list + ["--check", "--reverse", patch_path]
            check_proc = subprocess.run(
                check_cmd_list, capture_output=True, cwd=mod_path, encoding="utf-8"
            )
            if check_proc.returncode == 0:
                self.inf(f"patch already applied, skipping: {pth}")
                self.dbg("SKIP")
                continue
    ...
    def clean(self, args, yml, dst_mods=None):
        clean_cmd = yml.get("clean-command", "")
        checkout_cmd = yml.get("checkout-command", "")
