            ...
            if filename.endswith('.elf') and trailer_flag == True:
                ...
                while i < len(section_lines) - 1:
                    ...
                    if len(parts) > 1 and parts[0].isdigit():
                        ...
                        if code_data_flags.intersection(flags):
                            #print("[INFO] Processing section: {0}".format(section_name))
                            temp_bin = "temp_{0}.bin".format(section_name.replace('.', ''))
                            try:
                                subprocess.check_call(["arm-none-eabi-objcopy", "-O", "binary", "--only-section={0}".format(section_name), filename, temp_bin])
                                with open(temp_bin, 'rb') as temp_file:
                                    ...
                                    for l in section_lines:
                                        if section_name in l:
                                            lma = int(l.split()[4], 16)
                                            vma = int(l.split()[3], 16)
                                            break
                                    ...
                                    segment_info = struct.pack(
                                        segment_info_format,
                                        0, 1, current_offset, section_length, lma
                                    )
                                    need_copy = 1 #if classify_memory_region(vma) != 1 else 0
                                    segment_infos.append((classify_memory_region(vma), need_copy, current_offset, section_length, lma))
                            ...
                            #print(f"[INFO] Skipping section: {section_name} flags: {flags}")
                            continue
