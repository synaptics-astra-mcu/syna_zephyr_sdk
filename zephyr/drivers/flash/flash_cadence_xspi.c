				  ...
				  uint32_t dummy_cnt)
{
	...
	LOG_DBG("Direct read sequence configured with cmd=0x%02x, io=%d, dummy=%d", reg_val & 0xFF,
		(reg_val >> XSPI_CTRL_READ_SEQ_CFG_0_DATA_IOS_POS) & 0x3,
		(reg_val >> XSPI_CTRL_READ_SEQ_CFG_0_DUMMY_CNT_POS) & 0xFF);
}
...
static void xspic_direct_stat_cfg(struct cad_xspi_params *cad_params)
{
	...
	LOG_DBG("Direct status sequence configured with RDSR cmd=0x%02x",
		(XSPI_OP_RDSR << XSPI_CTRL_STAT_SEQ_CFG_2_PROG_FAIL_CMD_VAL_POS) | XSPI_OP_RDSR);
}
...
static void xspic_copy(void *dest, const void *src, size_t len)
{
	...
	if (((uint32_t)src & 7) && ((uint32_t)dest & 7)) {
		memcpy(dest, src, len);
		return;
	}

	while ((uint32_t)b_src & 7 && (len > 0)) {
		*b_dest++ = *b_src++;
		len--;
	}

	while ((uint32_t)b_dest & 7 && (len > 0)) {
		*b_dest++ = *b_src++;
		len--;
	}
...
static int flash_cad_xspi_erase(const struct device *dev, off_t offset, size_t len)
{
	...
	if ((offset % SPI_NOR_SECTOR_SIZE) || (offset < 0) || offset + len > cad_params->xip_size) {
				...
				size_t len)
{
	...
	while (len) {
		if (config->use_direct_mode) {
			...
			xspic_copy((void *)(cad_params->xip_base + offset), src, write_size);
			barrier_dsync_fence_full();
...
static int flash_cad_xspi_read_jedec_id(const struct device *dev, uint8_t *id)
{
	...
	acquire_device(dev);
	ret = xspic_stig_read_data(cad_params, JESD216_CMD_READ_ID, 0, id, JESD216_READ_ID_LEN, 0,
				   0);
	release_device(dev);
...
static int flash_cad_xspi_init(const struct device *dev)
{
	...
	sys_write32(0xf90006, cad_params->reg_base + XSPI_CTRL_WE_SEQ_CFG_0);
...
#else
#define CLOCK_XSPI_CONFIG(n) (.clk_dev = NULL,)
#endif
...
#define PAGE_LAYOUT(n)                                                                             \
	.pages_layout = {                                                                          \
		.pages_count = DT_INST_REG_SIZE_BY_IDX(n, 1) / SPI_NOR_SECTOR_SIZE,                \
