...
static void setup_soc(void)
{
    ...
    sys_write32(2, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_topStickyResetN)); /* NPU */
}
...
void soc_ddr_init(int real)
{
	ddr_config_t ddr_config = {};

	DDR_INIT_CONFIG(&ddr_config, 1 /*CONFIG_DDR_TYPE*/, false);
	(void)sys_ddr_init(&ddr_config, real);
}
#endif

#if DT_NODE_HAS_STATUS(DT_NODELABEL(mac0), okay) || DT_NODE_HAS_STATUS(DT_NODELABEL(mac1), okay)
...
static void enable_ethernet(void)
{
	...
	/* Set SYSPLL1 to 1GHz */
	sys_write32(0x002, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_ClkSwitch)); /* bypass */
	sys_write32(0x012, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1));
	sys_write32(0x013, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1));
	sys_write32(0x000, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1 + 0x08));
	sys_write32(0x027, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1 + 0x0c));
	sys_write32(0x000, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1 + 0x10));
	sys_write32(0x000, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1 + 0x04));
	sys_write32(0x01b, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1));
	sys_write32(0x001, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1 + 0x14));
	sys_write32(0x004, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1 + 0x18));
	busy_loop();
	sys_write32(0x01a, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1));
	busy_loop();
	sys_write32(0x018, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_sysPll_1));
	busy_loop();
	sys_write32(0x000, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_ClkSwitch));

#if DT_NODE_HAS_STATUS(DT_NODELABEL(mac0), okay)
	/* GE0 at 250 MHz */
	sys_write32(0x136, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_gethRgmiiClk));
	sys_write32(0x137, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_gethRgmiiClk));
#endif
#if DT_NODE_HAS_STATUS(DT_NODELABEL(mac1), okay)
	/* GE1 at 250 MHz */
	sys_write32(0x136, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_gethRgmii1Clk));
	sys_write32(0x137, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_gethRgmii1Clk));
#endif

	/* MDIO-MUX */
	value = sys_read32(SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_PERIF +
					 RA_PERIF_TW_PORT_CTRL));
	value |= BIT(4);
	sys_write32(value, SOCREG_REGION(START_CHIP_CTRL_REG + RA_Gbl_PERIF +
					 RA_PERIF_TW_PORT_CTRL));

	/* Interrupt - ICTL0 */
	value = sys_read32(SOCREG_REGION(START_ICTL0_REG));
#if DT_NODE_HAS_STATUS(DT_NODELABEL(mac0), okay)
	value |= BIT(15); /* GE0 */
#endif
#if DT_NODE_HAS_STATUS(DT_NODELABEL(mac1), okay)
	value |= BIT(16); /* GE1 */
#endif
	sys_write32(value, SOCREG_REGION(START_ICTL0_REG));
}
#endif /* mac0 || mac1 */
