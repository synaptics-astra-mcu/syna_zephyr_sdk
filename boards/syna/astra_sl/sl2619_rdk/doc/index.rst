...
Initialize the workspace folder (``syna_zephyr``). Make sure your ssh keys for accessing the
Synaptics repositories are added to github and authenticated. Run the following command:
...
Alternative: Flash the Zephyr Image onto eMMC via U-Boot
--------------------------------------------------------

The Zephyr firmware can be installed on the eMMC instead of the SPI flash of
the SL261x, replacing the preloader (M52 bootloader). The corresponding image
``build/image/eMMCimg/preboot.subimg.gz`` is automatically generated as part of
the ``west build`` invocation.

As a pre-requisite, install the Astra images as explained in step 2 above.

To install the Zephyr firmware image, boot into u-boot from eMMC, the internal
SPI flash, or via the USB boot tool. Install the file
``build/image/eMMCimg/preboot.subimg.gz`` on the partition ``b1``, e.g., via
network/TFTP and reset the board:

.. code-block:: console

   => net_init
   => dhcp
   => img2sd preboot.subimg.gz b1

Please note that the boot flow will stop with the Zephyr firmware, i.e.,
subsequent images like u-boot, Linux kernel etc. will not be loaded anymore.
If you want to update the preboot partition (either with a new Zephyr firmware
or the original preboot image from the Astra release) but into u-boot on
the internal SPI flash (set the ``SD_BOOT`` jumper and perform a reset while
``USB_BOOT`` is pressed), download and update the ``b1`` partition with the
corresponding image as documented above (``img2sd`` command).
...
1. Flash eMMC (Linux)
2. Boot A55 Linux
3. Flash Zephyr to XSPI or eMMC
4. Enable SPI boot (SD_BOOT) in case of XSPI
5. Reset board
6. Zephyr runs
