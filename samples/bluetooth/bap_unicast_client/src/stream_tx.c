...
static struct tx_stream tx_streams[CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT];
	...
	while (true) {
		...
		for (size_t i = 0U; i < ARRAY_SIZE(tx_streams); i++) {
			...
			if (stream_is_streaming(bap_stream)) {
				...
				if (buf == NULL) {
					/* No buffer available for this stream right now.
					 * Skip so the other stream is not starved.
					 */
					continue;
				}
				...
				int err = bt_bap_stream_send(bap_stream, buf, tx_streams[i].seq_num);
		...
		if (!any_sent) {
			/* No stream was able to send — wait briefly for buffers or stream state */
			k_sleep(K_MSEC(1));
		}
	}
}
