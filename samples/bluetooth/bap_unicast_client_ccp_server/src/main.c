...
static app_action_t current_action = ACTION_NONE;

/* Per-stream codec presets — music (stereo MEDIA) */
static struct bt_bap_lc3_preset media_presets[CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT] = {
...
/* Per-stream codec presets — call (CONVERSATIONAL) */
static struct bt_bap_lc3_preset call_presets[CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT] = {
	BT_BAP_LC3_UNICAST_PRESET_16_2_1(BT_AUDIO_LOCATION_FRONT_LEFT,
...
/* Points to whichever preset set is active; set in main() before BAP setup */
static struct bt_bap_lc3_preset *sink_codec_cfg = media_presets;
...
static struct bt_ccp_call_control_server_bearer
	*ccp_bearers[CONFIG_BT_CCP_CALL_CONTROL_SERVER_BEARER_COUNT];

/* ── Interactive action menu ─────────────────────────────────────── */
static app_action_t select_action(void)
{
	char *line;

	while (true) {
		printk("\n=== Select action ===\n");
		printk("  [1] Play music  (BAP Unicast, MEDIA context)\n");
		printk("  [2] Incoming call (CCP simulate + BAP Unicast, CONVERSATIONAL)\n");
		printk("  q:  Disconnect\n");
		printk("Choice: ");
...
{
	static const struct bt_tbs_register_param gtbs_param = {
	...
	err = bt_ccp_call_control_server_register_bearer(&gtbs_param, &ccp_bearers[0]);
	if (err != 0) {
		printk("Failed to register GTBS bearer (err %d)\n", err);
		return err;
	}

#if CONFIG_BT_CCP_CALL_CONTROL_SERVER_BEARER_COUNT > 1
	{
		static const struct bt_tbs_register_param tbs_param = {
			.provider_name = "TBS",
			.uci = "un001",
		...
		err = bt_ccp_call_control_server_register_bearer(&tbs_param, &ccp_bearers[1]);
		if (err != 0) {
			printk("Failed to register TBS bearer (err %d)\n", err);
			return err;
		}
	}
#endif

	printk("CCP Call Control Server initialized\n");
	return 0;
}
...
static int create_group(void)
{
	...
	for (size_t i = 0U; i < configured_stream_count; i++) {
		...
		stream_params[i].qos = &sink_codec_cfg[MIN(i, CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT - 1)].qos;
	}
...
static int enable_streams(void)
{
	for (size_t i = 0U; i < configured_stream_count; i++) {
		...
		const size_t cfg_idx = MIN(i, CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT - 1);
...
int main(void)
{
	...
	while (true) {
		...
		/* Set the codec preset pointer before BAP setup */
		sink_codec_cfg = (current_action == ACTION_CALL) ? call_presets : media_presets;
		...
		/* Simulate incoming call via TBS after GATT discovery so the peer
		 * has had time to subscribe to TBS CCCDs. Without this delay the
		 * notifications are dropped because the peer hasn't written the
		 * CCCDs yet and Zephyr only notifies subscribed clients.
		 */
		if (current_action == ACTION_CALL) {
			k_sleep(K_MSEC(500));
			int call_idx = bt_tbs_remote_incoming(BT_TBS_GTBS_INDEX,
							      "tel:+886912345678",
							      "tel:+886987654321",
							      "Alice");
			if (call_idx < 0) {
				printk("Failed to simulate incoming call (err %d)\n", call_idx);
			} else {
				printk("Simulated incoming call index=%d\n", call_idx);
			}
		}
		 ...
		 * STREAMING state. Delaying TX start prevents radio-level
		 * interference between CIS 0x60 data transmissions and CIS 0x61
		 * connection setup.
		 */
		if (IS_ENABLED(CONFIG_BT_AUDIO_TX)) {
			for (size_t i = 0U; i < configured_stream_count; i++) {
